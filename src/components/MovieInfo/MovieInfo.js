...
class MovieInfo extends Component {
    ...
    return (
      <div className="details" style={{ width: "100%" }}>
        ...
        <iframe
          title={title}
          className="trailer"
          src={`https://videospider.in/getvideo?key=Eqvv2uJrorySaQeS&video_id=${socials.imdb_id}`}
      ...
      <div className="details" style={{ width: "100%" }}>
        ...
        <div className="general">
          ...
          >
            Revenue:{" "}
            {revenue !== 0
              ? String(revenue).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
              : "pending"}
          </p>
          <p
            className="generalText"
            ...
            {actors.map((item, i) => {
              return (
                <div className="actorBox" key={i} ref={i}>
                  ...
                  <p className="actorChar">{item.character}</p>
                </div>
              );
            })}
          </div>
  ...
  renderRecommended() {
    ...
    const scrollIndex2 = index2 + 3 >= recommendations.length ? 2 : index2 + 3;
    if (recommendations.length === 0) {
      return (
        <div className="details" style={{ width: "100%" }}>
          <p className="detailsTitle">Recommended</p>
          <p className="overviewText">Recommendations not available</p>
        </div>
      );
    }
    return (
          ...
          return (
            <div className="revCont" key={item.userId}>
              <div className="revBox">
                ...
                <Link to={`/profile/${item.username}`} className="revDtlsText">
                  By: {item.username}
                </Link>
              </div>
              ...
              <div className="likeBox">
                <p
                  className="revText"
                  style={{
                    color: liked.includes(item.id) ? "#FFCA28" : "#ffffff49",
                    margin: 0
                  }}
                >
                  {item.liked.length}
