        ...
        <div className="formItemBox">
          ...
          {item.title !== "Password" ? (
            <input
              type="text"
              value={
                item.title === "Email" ? this.state.email : this.state.name
              }
              onChange={
                item.title === "Email" ? this.handleEmail : this.handleName
              }
              className="formInput"
      ...
      <Layout className="routeBox" user={this.props.user}>
        {redirect ? (
          ...
          <div className="loginBox">
            ...
            <div className="credentialsBox">{this.renderForm(formData)}</div>
            ...
            >
              ...
              <div className="modalBox">{this.renderCategories()}</div>
              <button
                className="modalDone"
                style={{
                  opacity: this.state.selectedGenres.length !== 3 ? 0.5 : 1
                }}
                onClick={this.hide}
                disabled={this.state.selectedGenres.length !== 3}
              >
                Done
              </button>
            </Rodal>
            ...
            >
              <p className="modalTitle" style={{ color: "#F44336" }}>
                Error!
              </p>
              <div className="errorBox">
                <p className="errorMessage">{errors.message}</p>
                <button className="modalDone" onClick={this.hideError}>
                  OK
                </button>
              </div>
            </Rodal>
