            ...
            <div className="feedRevCont" key={item.userId}>
              ...
              <div className="feedLikeBox">
                <p
                  ...
                  style={{
                    color: liked.includes(item.id) ? "#F44336" : "#cccccc",
                    margin: 0
                  }}
                >
                  {item.liked.length}
                ...
                >
                  <button
                    ...
                    onClick={() => this.handleLike(item.id, item.movieId)}
                  >
                    <MdThumbUp />
                  </button>
                </IconContext.Provider>
  ...
  render() {
    ...
    const genres =
      user.categories.length > 3
        ...
        : user.categories.slice(0, 3);
    const activeIndex =
      active !== "Review" ? genres.findIndex(item => item.name === active) : 3;
        ...
        {redirect ? (
          ...
          <div className="profileBox">
            <div
              className="userBox"
              style={{
                position: width > maxWidth * 0.8 ? "fixed" : "relative"
              }}
            >
              <p className="usernameText">{user.name}</p>
              <div className="accBtn">
                <button className="feed" onClick={this.handleFeed}>
                  Feed
                </button>
                <button className="acc" onClick={this.handleAcc}>
                  Account
                </button>
                <button className="acc" onClick={this.handleLogout}>
                  Log Out
                </button>
              </div>
            </div>
              ...
              <div className="profileContBox">
                <div className="userGenres">
                  <div
                    ...
                    style={{
                      borderColor: this.sectionColor(activeIndex),
                      position: width > maxWidth * 0.8 ? "fixed" : "relative"
                    }}
                  >
                    {genres.map((item, index) => {
                      return (
                        <button
                          ...
                          style={{
                            backgroundColor: this.sectionColor(index)
                          }}
                        >
                          {item.name}
                        </button>
                    ...
                    <button
                      ...
                      style={{
                        backgroundColor: this.sectionColor(3),
                        marginTop: 40
                      }}
                    >
                      Reviews
                  ...
                  <div className="filmList">
                    {genresData !== undefined && active !== "Review"
                      ? genresData[active].map(item => {
                          var bookmark =
                            userBookmarks !== null
                              ...
                              : null;
                          return (
                            <div className="filmCont" key={item.id}>
                              <Poster
                                item={item}
                                size={154}
                                margin={10}
                                ...
                                radius="8px"
                              />
                              <p className="filmText">{item.title}</p>
                            </div>
                          );
                        })
                      : this.renderReviews()}
              ...
              <div className="userInfoBox">
                <div className="userInfoCont">
                  <div className="infoTextBox">
                    ...
                    <p className="userInfoText">Categories:</p>
                    <p className="userInfoText">Followers:</p>
                    <p className="userInfoText">Follows:</p>
                    <p className="userInfoText">Created At:</p>
                  </div>
                  <div className="infoTextBox">
                    <p className="userInfoText">{user.name}</p>
                    <p className="userInfoText">{user.email}</p>
                    <p className="userInfoText">
                      {user.categories[0].name}, {user.categories[1].name},{" "}
                      {user.categories[2].name}
                    </p>
                    <p className="userInfoText">
                      {user.followers.length} users
                    </p>
                    <p className="userInfoText">{user.follows.length} users</p>
                    <p className="userInfoText">{user.createdAt}</p>
                  </div>
                </div>
              </div>
            )}
